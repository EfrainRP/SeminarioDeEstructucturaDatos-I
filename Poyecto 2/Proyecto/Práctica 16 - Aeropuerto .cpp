...
void Nodo::comprarBoleto(Nodo *&frente, Nodo *&fin, int num){
	...
	srand(time(NULL));
	int random;
	cout<<endl<<"Nombre de Usuario "<<num<<": ";
	cin>>nuevo_nodo->nombre;
	nuevo_nodo->maletas = random;
...
int Nodo::recogerBoleto(Nodo *&frente, int cont){
	...
	do{
		cout<<"Pasajero "<<temp->dato<<" | "<<temp->nombre<<" | Maletas "<<temp->maletas<<"    "<<endl;
		temp = temp->siguiente;
	}while(temp != NULL);
...
void Nodo::asientoReservado(Nodo *&frente, int cont){
	...
	while(temp!=NULL){
		random = 1+rand()%(101-1);
		cout<<"\n\tPasajero "<<temp->dato<<": "<<endl<<endl;
		cout<<"Nombre: "<<temp->nombre<<endl;
		cout<<"Equipaje: "<<temp->maletas<<" maletas"<<endl;
...
void Nodo::revisionMaletas(Nodo *&frente, int cont){
	...
	while(temp!=NULL){
		cout<<"\nPasajero "<<temp->dato<<" | "<<"Nombre: "<<temp->nombre<<" | "<<"Cantidad de Maletas: "<<temp->maletas;
		temp = temp->siguiente;
	}
...
void Nodo::Menu(){
	...
	do{
		...
		if(validarDatos(cant) == true){
			...
			cout<<"\nEl Dato Ingresado no es un Numero. . ."<<endl<<endl;
			fflush(stdin);
			system("pause");
			system("cls");
		}
	}while(validarDatos(cant) == false);
	system("pause");
}
